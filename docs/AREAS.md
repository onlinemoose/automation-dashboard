...
| Bucket | Owns |
|---|---|
| **Shell** | `dashboard/app.py` (routing skeleton, `/p/{slug}`, `create_app`), `dashboard/_auth.py`, `dashboard/_access.py` (per-user area grants — `docs/ACCESS.md`), `dashboard/_render.py` (incl. `make_templates`), `dashboard/_streaming.py` (`stream_run`), `dashboard/pages/_spec.py` (incl. `Area`), `dashboard/pages/__init__.py`, `dashboard/areas/__init__.py`, `dashboard/hashpw.py`, `templates/{base,login,index,page,result,_result_panel,_running_*}.html` (the nav in `base`/`index`/`_running_open` iterates `area_nav`, filtered per-user by `_access.visible_areas`), `static/app.css`, `docs/{EXPERIENCE,DEPLOY,DEPLOYMENT_CHECKLIST,USER_SCOPING,ACCESS,AREAS}.md`, `tests/test_{pages,auth,auth_backend,guardrails}.py`. |
...
| **Content Creation Team** (entry `/content`) | `dashboard/areas/content_creation_team/**` — self-contained: `pages/content_creation_team.py` (+ `pages/_examples/`), `_briefs.py`, `_content_team.py`, `_content_drafts.py`, `_content_targeted_edit.py`, `_publish.py`, `_feldklang_repo.py`, `routes.py` (`APIRouter`, `/content*`), `templates/`, `docs/CONTENT_BRIEFS.md`, nested `CLAUDE.md`. Also `static/content-draft-edit.js` (in `dashboard/static/`, the single mount). `tests/test_content_{briefs,drafts,publish}.py`. Capabilities: content-creation-team, targeted-editor, publish-to-website. `_feldklang_repo.py` is the area's own outbound GitHub commit client (App-owned integration, CLAUDE.md rule 7 territory, not a capability) — it pushes the `.mdx` + hero image `publish-to-website` returns to `onlinemoose/feldklang`. |
