...
| **Content Creation Team** | `/content` | `dashboard/areas/content_creation_team/**` (self-contained: `pages/`, `_briefs.py`, `_content_team.py`, `_content_drafts.py`, `_content_targeted_edit.py`, `_publish.py`, `_feldklang_repo.py`, `routes.py`, `templates/`, `docs/CONTENT_BRIEFS.md`, nested `CLAUDE.md`), `static/content-draft-edit.js`, `tests/test_content_{briefs,drafts,publish}.py`. Capabilities: content-creation-team, targeted-editor, publish-to-website. `_feldklang_repo.py` is this area's own outbound GitHub commit client (not a capability) — it pushes the files `publish-to-website` returns to `onlinemoose/feldklang`. |
