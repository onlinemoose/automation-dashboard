...
(function () {
  ...
  function propose() {
    ...
    clearError();
    setBusy(true);
    fetch("/content/drafts/" + draftId + "/revise", {
      method: "POST",
  ...
  acceptBtn.addEventListener("click", function () {
    ...
    fetch("/content/drafts/" + draftId + "/accept", {
      method: "POST",
      headers: { "Content-Type": "application/x-www-form-urlencoded" },
      body: form({
        selection: pending.selection,
  ...
  if (undoBtn) {
    undoBtn.addEventListener("click", function () {
      ...
      saveDoc()
        .then(function () {
          return fetch("/content/drafts/" + draftId + "/undo", { method: "POST" });
        })
        .then(function (r) {
  ...
  function saveDoc() {
    ...
    clearDocError();
    return fetch("/content/drafts/" + draftId + "/edit", {
      method: "POST",
      headers: { "Content-Type": "application/x-www-form-urlencoded" },
      body: form({ text: text }),
    })
