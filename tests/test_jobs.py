...
def test_analyse_api_failure_keeps_the_emphasis_and_warns(
    client: TestClient, monkeypatch: pytest.MonkeyPatch
) -> None:
    job = _jobs.create_job_post("Acme — Product Lead", POSTING, USER)
    _jobs.update_job_post(job.id, USER, emphasis="My hand-written notes\n- keep these")

    def boom(data: job_analyst.Input) -> job_analyst.Output:
    ...
    monkeypatch.setattr(job_analyst, "run", boom)

    resp = client.post(f"/jobs/{job.id}/analyse")
    assert resp.status_code == 502
    assert "ANTHROPIC_API_KEY" in resp.text
    assert "invalid x-api-key" not in resp.text  # the raw SDK message is not leaked
    assert _jobs.get_job_post(job.id, USER).emphasis == "My hand-written notes\n- keep these"
